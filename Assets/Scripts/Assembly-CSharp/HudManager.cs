...
public class HudManager : DestroyableSingleton<HudManager>
{
	...
	public void Start()
	{
		...
		SetJoystickSize(SaveManager.JoystickSize);
	}
	...
	public void SetTouchType(int type)
	{
		...
		if (!IsMobilePlatform())
		{
			...
			if (joystick == null)
			{
				GameObject gameObject = new GameObject("KeyboardJoystick");
				gameObject.transform.SetParent(base.transform, worldPositionStays: false);
				joystick = gameObject.AddComponent<KeyboardJoystick>();
			}
			return;
		}
		CreateJoystick((type == 1) ? 1 : 0);
		SetJoystickSize(SaveManager.JoystickSize);
	}
	...
	{
		if (Joysticks == null || index < 0 || index >= Joysticks.Length || Joysticks[index] == null)
		{
			return;
		}
		MonoBehaviour monoBehaviour = UnityEngine.Object.Instantiate(Joysticks[index]);
	...
	{
		UpdatePlayerShadowClip();
	}

	private void UpdatePlayerShadowClip()
	{
		if (!(bool)ShadowQuad || !ShadowQuad.gameObject.activeInHierarchy)
		{
			return;
		}
		Material material = ShadowQuad.sharedMaterial;
		Texture texture = material ? material.GetTexture("_MainTex") : null;
		if (!texture)
		{
			return;
		}
		Shader.SetGlobalTexture("_PlayerShadowClipTex", texture);
		Shader.SetGlobalMatrix("_PlayerShadowClipWorldToLocal", ShadowQuad.transform.worldToLocalMatrix);
	}
