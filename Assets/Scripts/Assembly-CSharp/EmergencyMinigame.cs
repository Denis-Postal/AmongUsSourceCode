...
public class EmergencyMinigame : Minigame
{
	...
	public void Update()
	{
		...
		{
			int b = Mathf.CeilToInt(15f - ShipStatus.Instance.Timer);
			b = Mathf.Max(Mathf.CeilToInt(ShipStatus.Instance.EmergencyCooldown), b);
			ButtonActive = false;
			StatusText.Text = CompactEmergencyText(DestroyableSingleton<TranslationController>.Instance.GetString(StringNames.EmergencyNotReady));
			NumberText.Text = b + "s";
		...
		else if (!PlayerControl.LocalPlayer.myTasks.Any(PlayerTask.TaskIsEmergency))
		{
			if (state != 1)
			{
				...
				StatusText.Text = CompactEmergencyText(DestroyableSingleton<TranslationController>.Instance.GetString(StringNames.EmergencyCount, PlayerControl.LocalPlayer.Data.PlayerName));
		...
		else if (state != 2)
		{
			state = 2;
			ButtonActive = false;
			StatusText.Text = DestroyableSingleton<TranslationController>.Instance.GetString(StringNames.EmergencyDuringCrisis);
	...
	public void CallMeeting()
	{
		...
		{
			StatusText.Text = DestroyableSingleton<TranslationController>.Instance.GetString(StringNames.EmergencyRequested);
	...
	private static string CompactEmergencyText(string text)
	{
