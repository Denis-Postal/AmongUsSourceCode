...
public class PlayerPhysics : InnerNetObject
{
	...
	public IEnumerator CoSpawnPlayer(LobbyBehaviour lobby)
	{
		if ((bool)lobby)
		{
			Vector3 spawnPos = Vec2ToPosition(lobby.SpawnPositions[myPlayer.PlayerId % lobby.SpawnPositions.Length]);
			...
			SoundManager.Instance.PlaySound(lobby.SpawnSound, loop: false).volume = 0.75f;
			Skin.SetSpawn();
			Skin.Flipped = rend.flipX;
			...
			yield return WalkPlayerTo((Vector2)spawnPos + vector);
	...
	public IEnumerator WalkPlayerTo(Vector2 worldPos, float tolerance = 0.01f)
	{
		...
		do
		{
			...
			body.velocity = vector2.normalized * Speed * num;
			yield return null;
		}
