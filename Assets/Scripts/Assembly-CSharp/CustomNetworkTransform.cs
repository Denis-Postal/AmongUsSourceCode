...
public class CustomNetworkTransform : InnerNetObject
{
	...
	{
		ushort minSid = (ushort)(lastSequenceId + 3);
		SnapTo(position, minSid);
	}
	...
	private void SnapTo(Vector2 position, ushort minSid)
	{
		if (SidGreaterThan(minSid, lastSequenceId))
		{
			...
			obj.position = (targetSyncPosition = vector);
			vector = (body.velocity = Vector2.zero);
			targetSyncVelocity = vector;
	...
	public override void Deserialize(MessageReader reader, bool initialState)
	{
		if (initialState)
		{
			...
			Vector3 vector = (base.transform.position = ReadVector2(reader));
