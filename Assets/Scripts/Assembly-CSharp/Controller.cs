...
public class Controller
{
	...
	private void UpdateTouches()
	{
		...
		int num = Mathf.Min(Input.touchCount, Touches.Length);
		for (int j = 0; j < num; j++)
		{
			Touch touch = Input.GetTouch(j);
			TouchState touchState2 = Touches[j];
			touchState2.Position = Camera.main.ScreenToWorldPoint(touch.position);
			touchState2.IsDown = touch.phase != TouchPhase.Ended && touch.phase != TouchPhase.Canceled;
		}
	...
	private void UpdateMouse()
	{
		...
		{
			TouchState touchState2 = Touches[i];
			touchState2.WasDown = touchState2.IsDown;
			touchState2.TouchStart = false;
			touchState2.TouchEnd = touchState2.IsDown;
			touchState2.IsDown = false;
		}
	}
