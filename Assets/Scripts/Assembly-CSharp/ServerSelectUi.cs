...
public class ServerSelectUi : MonoBehaviour
{
	...
	public void Start()
	{
		...
		for (int i = 0; i < availableServers.Length; i++)
		{
			if (!(availableServers[i].Name == "Custom"))
			{
				...
				if (availableServers[i] == DestroyableSingleton<ServerManager>.Instance.CurrentServer)
				{
		...
		if (DestroyableSingleton<ServerManager>.Instance.CurrentServer.Name == "Custom")
		{
			CustomServer.Select();
		}
	...
	internal void SelectServer(ServerSelector selected)
	{
		for (int i = 0; i < myButtons.Count; i++)
		{
			...
			if (serverSelector.MyServer != DestroyableSingleton<ServerManager>.Instance.CurrentServer)
			{
