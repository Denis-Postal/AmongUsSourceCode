...
public class ShipStatus : InnerNetObject
{
	...
	public Dictionary<SystemTypes, ShipRoom> FastRooms { get; private set; }
	...
	private void OnEnable()
	{
		if (Systems == null)
		{
			...
			if (IsHq)
			{
				...
				Systems.Add(SystemTypes.Reactor, new ReactorSystemType(45f));
			...
			else
			{
				...
				Systems.Add(SystemTypes.Reactor, new ReactorSystemType(30f));
	...
	private void Awake()
	{
		...
		FastRooms = AllRooms.Where((ShipRoom p) => p.RoomId != SystemTypes.Hallway).ToDictionary((ShipRoom d) => d.RoomId);
	...
	{
		List<GameData.PlayerInfo> list = (from pc in GameData.Instance.AllPlayers
			where !pc.Disconnected
			where !pc.IsDead
			select pc).ToList();
		...
		{
			int index = NextInfectedSelectionIndex(list.Count);
			array[num] = list[index];
			list.RemoveAt(index);
		}
		Debug.Log("Selected impostors ids=" + string.Join(",", array.Select((GameData.PlayerInfo player) => player.PlayerId.ToString()).ToArray()));
	...
	private static int NextInfectedSelectionIndex(int maxExclusive)
	{
		if (maxExclusive <= 1)
		{
			return 0;
	...
	public void Begin()
	{
		...
		for (byte b = 0; b < allPlayers.Count; b++)
		{
			hashSet.Clear();
	...
	private void GetAmbientSoundVolume(ShipRoom room, AudioSource player, float dt)
	{
		...
		Vector2 vector = (Vector2)room.transform.position + room.AmbientOffset;
		Vector2 truePosition = PlayerControl.LocalPlayer.GetTruePosition();
		float num = Vector2.Distance(vector, truePosition);
		if (num > room.AmbientMaxDist)
		{
			player.volume = 0f;
			return;
		}
		...
		float num3 = 1f - num / room.AmbientMaxDist - (float)num2 * 0.25f;
		player.volume = Mathf.Lerp(player.volume, num3 * room.AmbientVolume, dt);
	}
	...
	public override bool Serialize(MessageWriter writer, bool initialState)
	{
		...
		if (DirtyBits != 0)
		{
			...
			for (short num2 = 0; num2 < SystemTypeHelpers.AllTypes.Length; num2++)
			{
	...
	public override void Deserialize(MessageReader reader, bool initialState)
	{
		...
		for (short num3 = 0; num3 < SystemTypeHelpers.AllTypes.Length; num3++)
		{
