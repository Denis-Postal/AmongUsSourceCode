    ...
    public partial class WebSocket
    {
        ...
        public void Connect()
        {
            ...
            this.m_Socket.Log.Output = (ld, f) =>
                                       {
                                           var s = string.Format("WebSocketSharp: {0}: {1}", ld.Caller, ld.Message);
                                           switch (ld.Level)
                                           {
                                               case LogLevel.Trace:
                                               case LogLevel.Debug:
            ...
            if (this.m_Socket.IsSecure)
            {
                try
                {
                    this.m_Socket.SslConfiguration.EnabledSslProtocols = SslProtocols.Tls12;
                    this.m_Socket.SslConfiguration.TargetHost = this.Url.Host;
                    this.m_Socket.SslConfiguration.CheckCertificateRevocation = false;
                    this.m_Socket.SslConfiguration.ServerCertificateValidationCallback = (sender, certificate, chain, sslPolicyErrors) => true;
                }
                catch (Exception e)
                {
                    this.DebugReturn(DebugLevel.WARNING, "WebSocketSharp TLS config skipped: " + e.Message);
                }
            }
