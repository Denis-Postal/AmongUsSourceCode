...
namespace Assets.CoreScripts
{
	public class Telemetry : DestroyableSingleton<Telemetry>
	{
		...
		public void WriteMeetingStarted(bool isEmergency)
		{
			if (amHost)
			{
				Analytics.CustomEvent("MeetingStarted", new Dictionary<string, object> { { "IsEmergency", isEmergency } });
			}
		}
		...
		public void WriteMeetingEnded(float duration)
		{
			if (amHost)
			{
				Analytics.CustomEvent("MeetingEnded", new Dictionary<string, object> { { "IsEmergency", duration } });
			}
		}
		...
		public void WriteSabotageUsed(SystemTypes systemType)
		{
			if (gameStarted)
			{
				Analytics.CustomEvent("SabotageUsed", new Dictionary<string, object> { { "SystemType", systemType } });
			}
		}
		...
		public void WriteUse(byte playerNum, TaskTypes taskType, Vector3 worldPos)
		{
			if (gameStarted)
			{
				Analytics.CustomEvent("ConsoleUsed", new Dictionary<string, object> { { "TaskType", taskType } });
			}
		}
		...
		public void WriteCompleteTask(byte playerNum, TaskTypes taskType)
		{
			if (gameStarted)
			{
				Analytics.CustomEvent("TaskComplete", new Dictionary<string, object> { { "TaskType", taskType } });
			}
		}
		...
		internal void WriteDisconnect(DisconnectReasons reason)
		{
			if (gameStarted)
			{
				Analytics.CustomEvent("Disconnect", new Dictionary<string, object> { { "Reason", reason } });
			}
		}
		...
		public void EndGame(GameOverReason endReason)
		{
			if (gameStarted)
			{
				Dictionary<string, object> dictionary = new Dictionary<string, object> { { "Reason", endReason } };
				if (amHost)
				{
